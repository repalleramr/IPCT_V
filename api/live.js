...
module.exports = async function (req, res) {
  ...
  try {
      ...
      if (payload.match_state === "live") {
          ...
          // ==========================================
          // FIX 7 & 8: STABLE SLOTS + R(B) + SHIFTING STAR
          // ==========================================
          try {
              let b1Full = "";
              let b2Full = "";
              
              // 1. Rip the exact string "M Marsh 91(52)" and "R Pant 35(23)" directly from the <title> tag.
              // This guarantees we get the runs and balls accurately formatted without DOM noise.
              ...
              if (titleMatch && titleMatch[1]) {
                  ...
                  // Fallback: If title fails, hunt the bodyText for Name + Runs + (Balls)
                  ...
                  matches.forEach(m => {
                      ...
                      if (nameOnly.length > 2 && !nameOnly.toLowerCase().includes('total')) {
                          validBatters.push(`${nameOnly} ${m[2]}(${m[3]})`); // Rebuilds "Name Runs(Balls)"
                      }
                  });
              ...
              if (b1Full) {
                  // 2. Extract just the names to figure out who is on strike
                  let n1Match = b1Full.match(/([A-Za-z\s\.\-']+)/);
                  let n2Match = b2Full.match(/([A-Za-z\s\.\-']+)/);
                  let name1 = n1Match ? n1Match[1].trim() : "";
                  let name2 = n2Match ? n2Match[1].trim() : "";

                  // 3. Scan the Live HTML Table to see whose name is printed FIRST.
                  // Crex always puts the active striker on the top row.
                  let safeText = bodyText.replace(/([a-z])([A-Z])/g, '$1 $2').replace(/([a-zA-Z])(\d)/g, '$1 $2');
                  let batIdx = safeText.lastIndexOf("Batter");
                  if (batIdx === -1) batIdx = safeText.search(/Batsman/i);
                  let searchArea = batIdx !== -1 ? safeText.substring(batIdx, batIdx + 300) : safeText;

                  let pos1 = name1 ? searchArea.indexOf(name1) : -1;
                  let pos2 = name2 ? searchArea.indexOf(name2) : -1;

                  let isN1Striker = true; // Default to first slot
                  if (pos2 !== -1 && pos1 !== -1 && pos2 < pos1) {
                      isN1Striker = false; // Name 2 is on top of the list, so they are the striker
                  } else if (pos1 === -1 && pos2 !== -1) {
                      isN1Striker = false;
                  }

                  // 4. Lock the positions for visual stability and append the dynamic *
                  if (isN1Striker) {
                      payload.striker = b1Full + " *";
                      payload.non_striker = b2Full || "Off-Strike";
                  } else {
                      payload.striker = b1Full; // Striker slot stays visually still
                      payload.non_striker = b2Full + " *"; // Star shifts down
                  }
              } else {
