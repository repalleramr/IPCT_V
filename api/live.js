...
module.exports = async function (req, res) {
  ...
  // --- ADDED: 'winner' field for frontend interlocking ---
  let payload = {
  ...
  try {
    ...
    try {
      ...
      // ==========================================
      // AUTOMATIC FRONTEND WINNER INTERLOCK
      // ==========================================
      if (statusLower.includes('won by') || statusLower.includes('tied') || statusLower.includes('abandoned')) {
        ...
        if (statusLower.includes('won by')) {
          ...
          if (winMatch) {
            payload.winner = winMatch[1].trim();
          }
        } else if (statusLower.includes('tied')) {
          payload.winner = "TIED";
        } else {
          payload.winner = "NO RESULT";
        }
