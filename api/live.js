...
module.exports = async function (req, res) {
  ...
  try {
    ...
    // NODE 2: Cricbuzz Aggressive Scraper
    const scanPages = [
    ...
    // ==========================================
    // AGGRESSIVE META-DATA HUNTER (Venue & Toss)
    // ==========================================
    
    // 1. Grab Venue from Header
    payload.venue = $m('.cb-nav-subhdr').first().text().replace('Match Info', '').trim() || null;
    
    // 2. Deep Text Scan for Venue and Toss
    $m('.cb-col, .cb-font-14, .cb-text-gray, .cb-min-inf').each((i, el) => {
        let txt = $m(el).text().trim();
        
        // Hunt for Venue
        if (!payload.venue && txt.includes('Venue:')) {
            let v = txt.split('Venue:')[1].split(',')[0].trim(); // Grabs just the stadium name
            if (v.length > 3) payload.venue = v;
        }
        
        // Hunt for Toss
        if (!payload.toss && txt.includes('Toss:')) {
            let t = txt.split('Toss:')[1].split(/(?=Time|Venue)/)[0].trim(); // Grabs the toss result
            if (t.length > 3) payload.toss = t;
        }
    });
    ...
    if (scoreHeader && scoreHeader.match(/\d+\/\d+/)) {
        payload.live_score = scoreHeader.split('-')[0].trim(); 
