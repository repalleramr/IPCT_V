    ...
    if (payload.match_state === "live") {
      ...
      try {
        ...
        if (b1Full) {
          ...
          let n2LastWord = name2 ? name2.split(' ').pop().toLowerCase() : "xyz_none";

          // Strategy 1: Look for literal '*' or '🏏' physically attached to the name in DOM
          let starRegex2 = new RegExp(n2LastWord + "[^a-zA-Z]{0,5}[\\*🏏]", "i");
          let starRegex1 = new RegExp(n1LastWord + "[^a-zA-Z]{0,5}[\\*🏏]", "i");

          if (starRegex2.test(safeText)) {
              isN1Striker = false;
          } else if (!starRegex1.test(safeText)) {
              // Strategy 2: Positional Race (Who appears first right after the word 'Batter')
              let batIdx = safeText.search(/Batter|Batsman/i);
              let searchArea = batIdx !== -1 ? safeText.substring(batIdx, batIdx + 300).toLowerCase() : safeText.toLowerCase();
              
              let pos1 = searchArea.indexOf(n1LastWord);
              let pos2 = searchArea.indexOf(n2LastWord);
          ...
          // Assign the bat symbol dynamically
          if (isN1Striker) {
