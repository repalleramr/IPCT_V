...
module.exports = async function (req, res) {
  ...
  try {
      ...
      if (payload.match_state === "live") {
          ...
          // ==========================================
          // TRIPLE-THREAT STRIKER SHIFT ENGINE
          // ==========================================
          try {
              let b1Full = ""; let b2Full = "";
              
              // 1. Grab names from Title (includes asterisks if present)
              ...
              if (titleMatch && titleMatch[1]) {
                  ...
                  let searchArea = batIdx !== -1 ? safeText.substring(batIdx, batIdx + 800) : safeText;
              ...
              if (b1Full) {
                  ...
                  // Method A: Check for explicit asterisk in extracted names
                  if (b2Full.includes('*')) {
                      isN1Striker = false;
                  } else if (b1Full.includes('*')) {
                      isN1Striker = true;
                  } else {
                      // Method B: 800-Character Positional Race
                      let safeText = bodyText.replace(/([a-z])([A-Z])/g, '$1 $2').replace(/([a-zA-Z])(\d)/g, '$1 $2');
                      let batIdx = safeText.search(/Batter|Batsman/i);
                      let searchArea = batIdx !== -1 ? safeText.substring(batIdx, batIdx + 800).toLowerCase() : safeText.toLowerCase();
                      
                      let k1 = getSearchKey(b1Full);
                      let k2 = getSearchKey(b2Full);
                      
                      let p1 = k1 ? searchArea.indexOf(k1) : -1;
                      let p2 = k2 ? searchArea.indexOf(k2) : -1;

                      if (p2 !== -1 && p1 !== -1 && p2 < p1) {
                          isN1Striker = false; // Player 2 appeared first in the HTML list
                      } else if (p1 === -1 && p2 !== -1) {
                          isN1Striker = false;
                      }
                  }

                  // Clean asterisks out of names before applying the bat emoji
          ...
          try {
              if (payload.live_score.includes('/')) {
                  ...
                  if (scoreMatchClean) {
                      ...
                      // Account for the emoji in striker name extraction
