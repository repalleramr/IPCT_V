...
module.exports = async function (req, res) {
  ...
  // --- MACRO PITCH ANALYSIS DATABASE ---
  const venueProfiles = {
    "chennai": { type: "Spin/Slow", par: 165 },
    "chidambaram": { type: "Spin/Slow", par: 165 },
    "lucknow": { type: "Sluggish/Grip", par: 160 },
    "ekana": { type: "Sluggish/Grip", par: 160 },
    "mumbai": { type: "Batting Paradise", par: 195 },
    "wankhede": { type: "Batting Paradise", par: 195 },
    "ahmedabad": { type: "Balanced/Pace", par: 180 },
    "modi": { type: "Balanced/Pace", par: 180 },
    "bengaluru": { type: "Flat/Small Boundaries", par: 200 },
    "bangalore": { type: "Flat/Small Boundaries", par: 200 },
    "chinnaswamy": { type: "Flat/Small Boundaries", par: 200 },
    "kolkata": { type: "Pace/Bounce", par: 185 },
    "eden": { type: "Pace/Bounce", par: 185 },
    "jaipur": { type: "Balanced/Large", par: 175 },
    "mansingh": { type: "Balanced/Large", par: 175 },
    "delhi": { type: "Flat Deck", par: 195 },
    "jaitley": { type: "Flat Deck", par: 195 },
    "hyderabad": { type: "Flat/True Bounce", par: 195 },
    "rajiv": { type: "Flat/True Bounce", par: 195 },
    "punjab": { type: "Pace/Bounce", par: 175 },
    "mullanpur": { type: "Pace/Bounce", par: 175 },
      ...
      try {
        if (payload.live_score.includes('/')) {
          ...
          if (scoreMatchClean) {
            ...
            // =========================================================
            // NEW: MICRO-MATCHUP & PITCH INTELLIGENCE
            // =========================================================
            let pitchProfile = "Balanced/Standard";
            let expectedPar = 175;
            for (let key in venueProfiles) {
              if (payload.venue.toLowerCase().includes(key)) {
                pitchProfile = venueProfiles[key].type;
                expectedPar = venueProfiles[key].par;
                break;
              }
            }
            ...
            if (totalBalls === 0 && !isRealMarket) {
              ...
              if (maxProb > 80 || favPaise <= 25) {
                if (userPosition === favTeam) matchTactic += `[DIRECTIVE] 🟢 BOOK SET OPPORTUNITY. You have a position on ${favTeam}. EAT (Lay) ${favTeam} at ${layPaise}p to recover initial stake and guarantee a Green Book.`;
                else matchTactic += `[DIRECTIVE] 🟡 HOLD / NO ENTRY. Odds are too low (${favPaise}p) to Back ${favTeam} safely. Wait for a wicket spike.`;
              } else if (isChase) {
