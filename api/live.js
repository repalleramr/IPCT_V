  ...
  try {
    ...
    if (!htmlAcquired) {
      try {
        ...
        if (crexUrl) {
          ...
          // --- SVG BAT INTERCEPTOR: Translates UI icons into pure text markers ---
    ...
    if (payload.match_state === "live") {
      ...
      try {
        ...
        // --- NEW SVG/DOM-AWARE BATTER EXTRACTION ---
        matches.forEach(m => {
          let matchIndex = m.index;
          let matchLength = m[0].length;
          
          // Check the text immediately surrounding the name for our injected *BAT* string
          let context = searchArea.substring(Math.max(0, matchIndex - 12), Math.min(searchArea.length, matchIndex + matchLength + 12));
          let isStriker = context.includes('*BAT*') || context.includes('*');
        ...
        if (validBatters.length > 0) {
          let b1 = validBatters[0];
          let b2 = validBatters.length > 1 ? validBatters[1] : null;

          // If the SVG interceptor found EXACTLY one active batsman, use it!
          if (b1.isStriker && (!b2 || !b2.isStriker)) {
            ...
            // FALLBACK: Only use top-down table logic if icons were hidden
            let name1 = b1.text.split(' ')[0].toLowerCase();
            let name2 = b2 ? b2.text.split(' ')[0].toLowerCase() : "xyz";
            let topNameInTable = "";
            ...
            if (tableHeaderMatch && tableHeaderMatch[1]) {
              topNameInTable = tableHeaderMatch[1].trim().toLowerCase();
            }

            if (topNameInTable.includes(name2) && !topNameInTable.includes(name1)) {
              payload.striker = b2.text + " 🏏";
              payload.non_striker = b1.text;
            } else {
              payload.striker = b1.text + " 🏏";
