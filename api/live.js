...
module.exports = async function (req, res) {
  ...
  // NEW: Ledger/Position Tracking Parameter
        ...
        if (payload.live_score.includes('/')) {
          ...
          if (scoreMatchClean) {
            ...
            if (totalBalls === 0 && !isRealMarket) {
              ...
              // =========================================================
              // NEW LEDGER-AWARE LOGIC BRANCH 
              // =========================================================
              if (maxProb > 80 || favPaise <= 25) {
                // If the user's position matches the current overwhelming favorite...
                if (userPosition === favTeam) {
                  matchTactic += `[ANALYSIS] ${favTeam} is dominating. Market is highly skewed.|[DIRECTIVE] 🟢 BOOK SET OPPORTUNITY. You have a position on ${favTeam}. EAT (Lay) ${favTeam} at ${layPaise}p to recover initial stake and guarantee a Green Book.`;
                } else {
                  // If the user has NO position, or a position on the losing team...
                  matchTactic += `[ANALYSIS] ${favTeam} is dominating. Market is highly skewed.|[DIRECTIVE] 🟡 HOLD / NO ENTRY. Odds are too low (${favPaise}p) to Back ${favTeam} safely. Do not force a trade here without prior position.`;
                }
              } else if (isChase) {
