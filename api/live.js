...
module.exports = async function (req, res) {
  ...
  // =======================================================================
  // CORE EXTRACTION (PRESERVED EXACTLY AS YOUR WORKING JS)
  // =======================================================================
  try {
    ...
    let isChase = false;
    let batTeam = ""; // Exposing for Shadow Trader
    ...
    if (payload.match_state === "live") {
      ...
      try {
        ...
        if (scoreMatch) {
          ...
          batTeam = payload.live_score.split(' ')[0] || "Batting Team";
      ...
      // =========================================================================
      // HUMAN INTUITION MATRIX & SHADOW TRADER (ISOLATED)
      // =========================================================================
      try {
        if (payload.live_score.includes('/')) {
            ...
            if (scoreMatchClean) {
                ...
                let rrrVal = 0;
                if (isChase) {
                    rrrVal = parseFloat(payload.required_rr) || 0;
                ...
                let oddsRegex = new RegExp(`\\b(${teamsPattern})\\b[^0-9a-zA-Z]{0,15}(\\d{2})\\s+(\\d{2})\\b`, 'gi');
                let oddsMatches = [...bodyText.matchAll(oddsRegex)];
                
                if (oddsMatches.length > 0) {
                    let m = oddsMatches[0];
                    let p1 = parseInt(m[2]); let p2 = parseInt(m[3]);
                    if (Math.abs(p1 - p2) <= 3 && p1 > 0 && p2 < 100) {
                        let matchedTeam = m[1].toLowerCase();
                ...
                let matchTactic = isRealMarket ? `[LIVE MARKET ODDS] ${favTeam} is Favorite at ${displayOdds} Paise\n` : `[AWAITING MARKET ODDS] Market Offline.\n`;
                matchTactic += `[LIVE PITCH] ${livePitchRead}\n[MOMENTUM] ${momentum}${chaseContext}`;

                // === [ADD-ON: SHADOW TRADER (CONTRARIAN VALUE HUNTER)] ===
                let contrarianAdvice = "";
                if (isRealMarket) {
                    if (favPaise <= 20) {
                        // Cheap Lay Opportunity
                    ...
                    } else if (isChase && favTeam === batTeam && rrrVal > 10.0 && wkts >= 3) {
                        // Scoreboard Pressure Trap
                    ...
                    } else if (isChase && favTeam !== batTeam && wkts <= 3 && rrrVal <= 11.0) {
                        // Underdog Wickets in Hand
                    ...
                    } else if (!isChase && totalBalls > 90 && recentRR > 10.0) {
                        // 1st Innings Death Over Surge
                ...
                if (isChase) { 
                  ...
                  else if (isWeatherInterrupted) payload.prediction = "🔴 HARD STOP - RAIN DELAY (Targets Subject to Change)";
    ...
    // =========================================================================
    // QUANTUM HEDGE ENGINE (VIRTUAL MODULE)
    // =========================================================================
    try {
        let aiAdvice = "";
